...
- **カメラ映像を WiFi で配信する Web サーバ (step 8 = ゴール・ライブ版、WiFi を UART に逃がして高速化)**
  ...
  **本命＝ソフトウェア JPEG エンコード**（`/qvga.jpg`・`/vga.jpg` 等、[src/jpeg.rs](src/jpeg.rs)）: カメラ HW JPEG が
  ...
  ネイティブデコード）。ベースライン 4:2:0・整数 DCT（固定小数点 P=11）・標準量子化(Q80)/Huffman を no_std no-alloc で
  ...
  3点で高速化した: ① **DCT を even/odd 分解**（D[u][7-x]=(-1)^u·D[u][x] を使い乗算 64→32/8点に半減、出力は不変）、
  ② **逆数量子化**（フレーム頭で round(2^16/quant) を作り、係数ごとの除算を乗算+シフトに置換）、③ **キャッシュドコピー**
  （撮影フレームを無しエイリアス→キャッシュ有りに1回コピーしてから符号化。各画素を輝度+色差で ~2回読むので、L1 ヒットに
  すると無し AXI のレイテンシを払わずに済む）。実測 enc は **QVGA 127→76ms / VGA 488→281ms（約1.7倍速）**、出力は
  **QVGA 18.9KB・VGA 39.4KB**（旧 RGB565直送比でそれぞれ ~650ms→計184ms・~2770ms→計433ms）。QVGA ではエンコードが
  転送より小さくなり再び UART 律速に。整数 DCT を Loeffler に替えればさらに短縮可。デノイズ併用でごま塩が消え JPEG も縮む。
