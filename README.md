...
**未決着**: オンボード RGB 赤 LED（IO13）の点灯。駆動はしているが視認できていない
（4.7K で極端に暗い／"光った"観察は全部 UART・ESP32・書き込み中の活動 LED のニセ陽性
だった）。決着には IO13 のテスタ実測か、リファレンス（Arduino/MaixPy）が**正しいピンを
正しく駆動している**ことの検証が必要 —— [docs/finding-the-led.md](docs/finding-the-led.md)。

カメラ(DVP)と WiFi(オンボード ESP32)は到達可能だが既製の Rust ドライバは無い:
[docs/hardware-maixduino.md](docs/hardware-maixduino.md)。
