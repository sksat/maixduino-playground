...
- **カメラ映像を WiFi で配信する Web サーバ (step 8 = ゴール・ライブ版、WiFi を UART に逃がして高速化)**
  ...
  **QQVGA(57KB)≒0.65s / QVGA(230KB)≒1.0s / VGA(922KB)≒4.0s（K210 実測の配信時間）**。旧 SPI 版（下の step 7）は
  160×120 で ~6–8s/枚だったので、QQVGA 同士で 10倍速・解像度は VGA まで。**UART リンクが律速**なので
  （バイト数 ÷ 3Mbaud が下限）、リンクを **3 Mbaud**（K210 分周 195MHz/48=4.0625 ちょうど）まで上げた。
  フレームバッファは VGA 分（614KB）を確保し小解像度は先頭を使う。撮影画像には DVP 由来の横縞＋ごま塩ノイズ
  （この基板の DVP 信号品質の限界、step「UXGA JPEG」参照）が乗り、解像度切替直後は AE/AWB 収束前で緑かぶりが
  出るが連写で消える。転送はバイト正確（BMP はフル）。
