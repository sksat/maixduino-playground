...
- **カメラ映像を WiFi で配信する Web サーバ (step 8 = ゴール・ライブ版、WiFi を UART に逃がして高速化)**
  ...
  **本命＝ソフトウェア JPEG エンコード**（`/qvga.jpg`・`/vga.jpg` 等、[src/jpeg.rs](src/jpeg.rs)）: カメラ HW JPEG が
  ...
  ネイティブデコード）。ベースライン 4:2:0・整数 DCT（直接・固定小数点 P=11）・標準量子化(Q80)/Huffman を no_std no-alloc で
  自前実装（出力は CAP[1] を流用、テーブルは [tools/gen_jpeg_tables.py](tools/gen_jpeg_tables.py) 生成、色変換と整数係数は
  codex と詰めた）。実測 **QVGA 18.9KB / enc 127ms / 計 192ms（旧 RGB565直送 ~650ms の約3.4倍速）、VGA 39.4KB / enc 488ms /
  計 632ms（旧 ~2770ms の約4.4倍速）**＝UART を ~12-15倍削減、エンコード時間は転送節約に対して十分小さい。整数 DCT は
  直接版なので Loeffler に替えれば enc はさらに短縮可。デノイズと併用するとごま塩が消えて JPEG もさらに小さくなる。
