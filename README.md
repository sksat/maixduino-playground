...
- **カメラ映像を WiFi で配信する Web サーバ (step 8 = ゴール・ライブ版、WiFi を UART に逃がして高速化)**
  ...
  **QVGA 127→65ms / VGA 488→237ms（約2倍速）**、出力は **QVGA 19KB・VGA 37KB**（旧 RGB565直送比でそれぞれ
  ...
  ごま塩が消え JPEG も縮む。
  **デュアルコアで並列エンコード**（`&2=1`、ページに 2-core トグル、JPEG sw 専用）: K210 は RV64 コアが2つあるので、
  ...
  **UI**: 全モードをページのボタンで
  ...
  `?d=1`(denoise)・`?e=1`(RGB565直送)。ページに**現モードの直 URL を表示＋Copy ボタン**（http なので `execCommand('copy')`
  フォールバック）。その URL を curl すれば同じモードのフレームを撮り続けられる（モードはリクエスト毎にパス/クエリから判定。
  解像度は substring 順 qqvga→qvga→vga で判定、旧 `?r=`/`?j=` もフォールバックで生存）。
